...
output_folder <- "data/data_replicated_ns/" # Folder for saving the results, must end with "/"
...
# for(row in 1:nrow(mixtures01)){ #this loop is better for debugging
  ...
  noise_not_overwritten <- noise2 %>% 
    filter(is.na(contributors)) %>%  #these will be directly added to the resulting profile
    select(Marker, Allele, Height)
  
  noise_overwritten00 <- noise2 %>% #these need to be placed to the locations of the former allelic and stutter peaks
    filter(contributors==1)
  
  if(nrow(noise_overwritten00)==0){
    noise_overwritten <- data.frame() # check if there is something to swap first
  } else{
    noise_overwritten <- noise_overwritten00 %>% 
      
    #let's first figure out if they are overwritten by stutter/allelic peaks
    left_join(target_contr_GTs01 %>% 
                filter(SampleName==settings[settings$origin==noisefromthis, "target"]),
              by=join_by("Allele", "Marker")) %>% 
      
      #let's see which of the origin allelic peaks are now missing noise peaks (they were placed to other locations earlier)
      left_join(origin_contr_GTs01 %>% 
                  filter(SampleName==noisefromthis) %>% 
                  select(Marker, Allele, Allele1, Allele2, Stutter1, Stutter2) %>% 
                  rename(Allele_origin_empty = Allele), #this variable will show where to put the noise
                by = join_by(Marker, Allele1, Allele2, Stutter1, Stutter2)) %>% 
      arrange(Marker, Allele) %>% 
      
      group_by(Marker, Allele_origin_empty) %>% 
      mutate(test = n()) %>% # check here to see which ones multiply, we shall sample one from them
      ungroup() %>% 
      
      group_by(Marker, Allele_origin_empty) %>% 
      summarise(Height = sample(Height, 1)) %>%  #if one origin allele has two possible heights, we sample one of them
      ungroup() %>% 
      
      rename(Allele = Allele_origin_empty)
    
    # TO DO:further checks that this noise stuff works properly! 
    if(noise_overwritten %>% 
       mutate(t = paste(Marker, "__", Allele)) %>% 
       filter(t %in% data_complete_listnoise$t) %>% 
       nrow() > 0){
      print(row)
      break
    }
  ...
  data_complete02 <- data_complete01 %>% 
    rbind(noise_overwritten) %>% 
    rbind(noise_not_overwritten) %>% 
